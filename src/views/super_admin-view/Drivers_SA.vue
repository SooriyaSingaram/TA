<template>
  <section id="Drivers_SA">
   ...
   <div class="vehicle_tlt d-flex flex-row">
            ...
            <h4>{{ 'Driver' | translate }}</h4>
                       
            <div class="sub_btn sub_bck_btn  d-flex justify-content-center drivers-btn">
                  <router-link to="/driver/0"><button class="cance_btn back_btn">{{ 'AddDriver' | translate }}</button></router-link>
            </div>
            </div>
          </div>
...
<el-dialog :visible.sync="centerDialogVisible" width="30%" center>
  ...
  <span slot="footer" class="dialog-footer">
    <el-button type="primary" v-on:click="centerDialogVisible = false; deleteDriver();" >{{ 'confirm' | translate }}</el-button> 
  ...
  data() {
   ...
   methods: {
    deleteDriver(){
			...
			var self = this;
      let corpData={url:'/SuperAdmin/DeleteDriver?DriverDetailsID='+self.driver_id}
			return this.$store.dispatch('updateData', corpData).then((response) => {
        this.$store.dispatch("getDriverLists");
			}).catch(function(err) {
    ...
    actionCol(){
       ...
       buttons: [{
          ...
          handler: row => {
            ...
            let id = row.DriverDetailsID
            this.$router.push({path: '/Driver_detail_SA/' +id})
          },
...

         {
          handler: row => {
            let id = row.DriverDetailsID
            this.$router.push({path: '/driver/' +id})
          },
