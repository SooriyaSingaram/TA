<template>
  <section id="Specialist_SA">
   <div class="container-fluid">
      <div class="row">
      <div class="col-12 main-title">
        <h2>{{ 'AdminFunctions' | translate }}</h2>
      </div>
       <div class="col-12 content_section">

          <div class="vehicle_tlt d-flex flex-row">
            <div class="col-12 d-flex  p-0">
            <h4>{{ 'Specialist' | translate }}</h4>
             <div class="sub_btn sub_bck_btn  d-flex justify-content-center drivers-btn">
               <router-link to="/specialist/0"><button class="cance_btn spcl_btn">{{ 'Addspecialist' | translate }}</button></router-link>
            </div>
            </div>
          </div>
         <div class="specialist_drop d-flex col-12 no-padding flex-wrap">
            <!--<div class="sel_fac col-lg-4 col-12 col-md-4 d-flex no-padding">
              <label>Filter by</label>
                <select class="form-control">
              <option>Specialist</option>
                </select>
            </div>
            <div class="sel_fac col-lg-4 col-12 col-md-4 d-flex no-padding">
              <label>Filter by</label>
                <select class="form-control">
              <option>Workshop</option>
                </select>
            </div>
             <div class="specialist-end-btn col-lg-4 col-12 col-md-4 d-flex no-padding">
               <router-link to="/specialist/0"><button class="cance_btn spcl_btn">Add Specialist</button></router-link>
            </div>-->
          </div>

        <div class="col-12 content_section_scroll p-0">
          <div class="content_section_table">
               <data-tables :data="this.data" :action-col="actionCol" >

      <el-table-column v-for="title in titles" :prop="title.prop" :label="title.label" :key="title.prop" sortable="custom">
      </el-table-column>
    </data-tables>
         <el-dialog :visible.sync="centerDialogVisible" width="30%" center>
         <h2 style="text-align: center; font-family: 'Nunito-Regular';font-weight: 600;font-size: 21px;color: black; margin-top: -30px; padding-bottom: 20px;">{{ 'Warning' | translate }}</h2>
  <span>{{ 'deleteAlert' | translate }}?</span>
  <span slot="footer" class="dialog-footer">
    <el-button type="primary" v-on:click="centerDialogVisible = false; deleteSpecialist();" >{{ 'Confirm' | translate }}</el-button>  
    <el-button type="danger" @click="centerDialogVisible = false">{{ 'CANCEL' | translate }}</el-button>
 </span>
</el-dialog>
            </div>
              
            </div>
            
            
  </div>
  </div>
  </div>
</section>
</template>

<script>

import { mapGetters, mapActions } from "vuex";
export default {
  name: 'Specialist_SA',
  components: {
    
  },
   data() {
    
    return {
        centerDialogVisible: false,

      
    }
  },

   methods: {
    deleteSpecialist(){
 
			var self = this;
      let specialistData={url:'/SuperAdmin/DeleteSpecialist?SpecialistDetailsID='+self.specialist_id}
			return this.$store.dispatch('updateData', specialistData).then((response) => {
        this.$store.dispatch("getSpecialistList");
			}).catch(function(err) {
				self.$message(err.response.data.error)
			});
		
    }
  
  },

 computed: {
     
    ...mapGetters({
      data: "getSpecialistList"
  
    }),
     titles(){return  [{
  prop: "id",
    label: "#"
    }, {
    prop: "UserName",
     label: this.$t('SpecialistName')
    }, {
    prop: "Email",
   label: this.$t('Email')
  },
  {
    prop: "PhoneNumber",
     label: this.$t('PhoneNumber')
  },{
    prop: "Address",
     label: this.$t('Address')
  }
  ]},
  actionCol(){ return {
        props: {
          label: this.$t('Action'),
          className: 'actionCenter'
        },
         buttons: [{
          props: {
            type: 'primary'
          },
          handler: row => {
           
            let id =+row.SpecialistDetailsID
            this.$router.push({path: '/Specialist_detail_SA/' +id})
          },
          label: <img src='data:image/png;base64,' />
        }, 

         {
          handler: row => {
            let id = row.SpecialistDetailsID
            this.$router.push({path: '/specialist/' +id})
          },
          label: <img src='data:image/png;base64,'/>
        },
        {
          handler: row => {
            this.centerDialogVisible = true;
             this.specialist_id=row.SpecialistDetailsID
          },
          label: <img src='data:image/png;base64,'/>
        }]
      }
  }

  },
	watch: {
		'getSpecialistList': {
			deep: true,
			handler(value) {
				this.data = null
				this.data = this.getSpecialistList;
        
			}
		}
	},
    created() {

      this.$store.dispatch("getSpecialistList");
      
    }

};
</script>

<style scoped>
 table.table thead th { border-top: none; } 
 thead th {  font-size: 15px;  padding: 12px;  text-transform: uppercase;  background: #0066d0;  color: #ffffff;}
 thead th { font-size: 15px;}
 tbody td {color: #434343; font-size: 16px;padding: 12px; }
.table-striped {  border-top-left-radius: 5px;  border-top-right-radius: 5px; overflow: hidden; }
.table-striped tbody tr:nth-of-type(2n+1) { background-color: #e3ebf3; }
.table-striped tbody tr {background: #eef6fe none repeat scroll 0 0;padding: 10px;}
.table td, .table th {vertical-align: middle; border-top: 1px solid #dee2e6; border-right: 1px solid #dee2e6; }
.table td:last-child,.table th:last-child { border-right: 0px solid #dee2e6;}
.bt-0 {border-top: 0!important;}
.table-striped tbody tr:last-child table tbody tr:nth-child(odd) {background: #eef6fe none repeat scroll 0 0;}
.table-striped tbody tr:last-child table tbody tr:nth-child(even) {background: #e3ebf3 none repeat scroll 0 0;}
td.action-ic img { width: 34px; height: 34px;cursor: pointer;}
td.action-ic { padding: 10px 10px!important;text-align: center;}
.content_section {background: #eef6fe;   padding: 30px;}
.content_section_scroll{overflow-x: auto;}
.content_section_table{width:100%;}
.pagination_ss{float:right; margin-top:20px;}
.pagination_ss mdb-page-item, .pagination_ss mdb-page-nav {
border: 1px solid #d3d7dc; font-size: 18px;padding: 6px 10px; background:#f3f6f9; color:#3a4959; border-radius:5px;
}
.pagination_ss mdb-page-item:hover, .pagination_ss mdb-page-nav:hover {
border: 1px solid #3a4959; font-size: 18px;padding: 6px 10px; background:#3a4959; color:#fff;  border-radius:5px;
}
.pagination_ss .active {
border: 1px solid #3a4959; font-size: 18px;padding: 6px 10px; background:#3a4959; color:#fff; border-radius:5px;
}
no-padding{padding:0px !important;}
.specialist_drop label{width:30% !important; margin-top: 5px;}
.specialist_drop .sel_fac{background: transparent;}
.vehicle_tlt{margin-bottom: 15px;}
.specialist_drop {margin-bottom: 30px;}
.specialist-end-btn { justify-content: flex-end;}
.spcl_btn {background: #2ecc71; margin-top: -5px;}
.specialist_drop .sel_fac select {
    background: #f3f6fb url(../../assets/images/down_arrow_to.png) !important;background-repeat: no-repeat !important;
    background-position: 88% 50% !important;    background-repeat: no-repeat !important;}
@media only screen and (min-width:320px) and (max-width:1250px){
  .content_section_table{width:1019px !important;}
}
@media only screen and (min-width:320px) and (max-width:767px){
.specialist_drop label {
    width: 50% !important;
}
.specialist-end-btn {
    justify-content: center;
    margin-top: 20px;
}
}


.drivers-btn{margin-left: auto;}
.drivers-btn .cance_btn{background: #2ecc71;width:auto !important; padding:19px 20px 23px 20px !important;}
</style>