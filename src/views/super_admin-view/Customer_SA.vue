...
<div class="vehicle_tlt d-flex flex-row">
            ...
            <h4> {{ 'CustomerList' | translate }}</h4>
            <div class="sub_btn sub_bck_btn  d-flex justify-content-center drivers-btn">
                  <router-link to="/customer/0"><button class="cance_btn back_btn">{{ 'AddCustomer' | translate }}</button></router-link>
            </div>
            </div>
          </div>
...
export default {
 ...
 mounted: function () { 
    ...
    this.$root.$on('lang', (text) => { // here you need to use the arrow function
     this.$i18n.set(text)
     debugger
    })
  },
   methods: {
     ...
     deleteCorporate(){
			...
			var self = this;
      let corpData={url:'/SuperAdmin/DeleteCarOwner/'+self.corp_id}
 ...
 computed: {
    ...
    prop: "CorporateAgreement",
    label: this.$t('CorporateAgreement')
  }, {
    prop: "PhoneNumber",
  ...
  },
  {
     ...
     return  {
        ...
        buttons: [{
          ...
          handler: row => {
            ...
            let id = row.CarOwnerID
            this.$router.push({path: '/Customer_detail_SA/' +id})
          },
	...
	watch: {
		...
		'getCustomerList': {
			...
			handler(value) {
				this.data = null
				this.data = this.getCustomerList;
        debugger
			}
		}
	},
