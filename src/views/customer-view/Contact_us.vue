      ...
      <div class="col-12 row contact_cv content_section d-flex">
        ...
        <span v-show="errors.has('email')" class="help is-danger">{{ 'emailValidation' | translate }}</span>
          ...
          <div class="form-group input-group">
            ...
            <textarea  name="message" v-validate="'required|alpha'" :class="{'input': true, 'is-danger': errors.has('message') }"   id="msg" class="form-control text-area-contact" placeholder="Lorem ipsum is a dummuy text of content..." cols="30" rows="4" v-model="contactdata.Message"></textarea>
              <div class="clearfix">
             <i v-show="errors.has('message')" class="fa fa-warning"></i>
            <span v-show="errors.has('message')" class="help is-danger">{{ 'textValidation' | translate }}</span>
            </div>
          </div>
          <div class="form-group"  >
            <button @click="AddContactData(contactdata)" type="button" class="btn btn-primary btn-cont-sub">{{ 'Submit' | translate }}</button>
          </div>
        </form>
