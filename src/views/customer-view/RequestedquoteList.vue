...
<label>{{ 'FilterByvehicle' | translate }} </label>
      <div class="sel_fac">
                    <select class="form-control"  name="CustomerVehicleID"  v-model="search"   v-validate="'required'"  >
                   
    <option v-for="data in dropdown" :value="data">{{data}}</option>
    ...
    <div class="col-12 content_section_scroll p-0">
          <div class="content_section_table">
                <data-tables :data="this.searchData" :action-col="actionCol" >
  ...
  methods: {
   	getRequestedQuoteParam() {
            ...
            //     console.log(localStorage.getItem('userId')); 
                ...
                // let userid = 2;
			          this.$store.dispatch("getrequestedquoteList", userid);
	}
  },
   ...
   computed: {
    ...
    ...mapGetters({
      data: "getrequestedquoteList"
    }),
       searchData() {
      this.dropdown=  [...new Set(this.data.map(item => item.CustomerVehicleName))];
     
      
      return this.data.filter(post => {
       
        return post.CustomerVehicleName.toLowerCase().includes(
    ...
    prop: "CustomerVehicleName",
   label:this.$t('VehicleName') 
    }, {
    ...
    actionCol(){return {
      ...
      buttons: [{
          ...
          handler: row => {
           ...
           // alert(id);
            this.$router.push({path: '/Requestedforquote/'+id})
          },
  	...
  	watch: {
		'getrequestedquoteList': {
			...
			handler(value) {
				this.data = null
				this.data = this.getrequestedquoteList;
