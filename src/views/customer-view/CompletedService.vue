...
export default {
  ...
  created() {
    let id = Number(localStorage.getItem("userId").slice(1, -1));

    this.$store.dispatch("getCompletedService", id);
    this.getData();
  },
  computed: {
    ...mapGetters({
      data: "getCompletedServicelist"
    }),
    titles(){
      return [
        {
  ...
  watch: {
    getCompletedServicelist: {
      ...
      handler(value) {
        this.data = null;
        this.data = this.getCompletedServicelist;
...
.pagination_ss mdb-page-item,
.pagination_ss mdb-page-nav {
...
.pagination_ss mdb-page-item:hover,
.pagination_ss mdb-page-nav:hover {
